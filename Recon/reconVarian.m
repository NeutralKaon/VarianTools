...
%
% JM, 2014.
% SEE ALSO: load_varian
% TODO: Add in containers.map support, retidy code, generalise 
% This should work for most commonly used sequences
...
OneD={'spuls'};
SegmentedTwoDee={'ge_2dseg_work_old','ge2d_seg_jjm'};
...
%try
    ...
    elseif any(strcmp(sequence,Cine))
        ...
        if nchannel > 1
            warning('Multi-coil CINE recon not yet implemented');
        end
        nread    = params.np/2;
        nphase   = params.nv;
        ...
        if nchannel >1
            nframes = params.noframes;%size(raw,2)/nslices/nphase;
            k=zeros(nread,nphase,nframes,nslices,nchannel);
            temp=zeros(nread,nphase.*nslices,nchannel);
            
            for i=1:nframes
                tmp=raw(:,i:nframes:end);
                temp(:,:,:)=reshape(tmp,nread,nphase,nchannel);
                for kx=1:nchannel
                    for j=1:nslices
                        k(:,:,i,j,kx)=temp(:,((j-1)*(end/nslices)+1):j*(end/nslices),kx);
                    end
                end
            end
            k=conj(k);
    ...
    elseif any(strcmp(sequence,CartesianThreeD))
        ...
        elseif (nchannels>1) && (narrays==0)
            ...
            image=fftnc(k,[1 2 3]);
        elseif narrays > 0
            warning('Arrayed multicoil 3D scan recon is not yet guaranteed to work.')
            k=reshape(raw,[nread nphase params.arraydim/(nchannels*nphase2) nchannels nphase2 ]);
    ...
    elseif any(strcmp(sequence,SegmentedTwoDee))
        ...
        if length(params.nt) == 1
            ne = length(params.te);
        end
        nc=sum(params.rcvrs{:}=='y');
        if length(params.nt) ~= 1
            ne = params.arraydim./nc;
        end
        ...
        if nc == 1 %Single coil
        ...
        else %Multicoil recon!
            
            raw = reshape(raw, [np nv nc ns ne]);
            
            rawa = 0*raw;
            try
                rawa(:, params.pelist + nv/2 + 1,:, :,:) = raw;
            catch
                rawa(:, params.pelist + nv/2 + 1-params.partialPE,:, :,:) = raw;
            end
            raw = rawa;
            ...
            try
                raw = crop(raw, [np nv nc ns ne]);
            catch
                try
                    raw = crop(raw, [np nv nc ns]);
                catch
                    raw = crop(raw, [np nv nc]);
                end
            end
            
            if  isfield(params,'partialPE')
                 if (params.partialPE ~= 0)
            ...
            varargout{1}=kf; 
            try
                kf=ifft2c(squeeze((calc_combined_mckenzie_n(fft2c(kf)))));
            catch
                kf=ifft2c(squeeze((calc_combined_mckenzie(fft2c(kf)))));
                %kf=ifft2c(squeeze(sos(fft2c(kf),3))); 
                warning('McKenzie multicoil recon failed!');
            end
        end
        
        %Do multicoil recon without pre-whitened data, which is BAD and
        %might screw up the phase, but, hey, it probably works.
    ...
    elseif any(strcmp(sequence,Flow))
        ...
        warning('Recon not yet implemented for sequence file %s',sequence);
        image=[];
        k=[];
        raw=raw;
        params=params;
